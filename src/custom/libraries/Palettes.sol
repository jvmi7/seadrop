...
library Palettes {
    ...
    function getColorPalette(uint8 paletteIndex) internal pure returns (ColorPalette memory) {
        ...
        if (paletteIndex == Constants.WARM) {
            return
                ColorPalette({
                    name: "warm",
                    background: BACKGROUND,
                    surface: SURFACE,
                    barColors: ["#FF0000", "#FF6200", "#FFE300", "", "", "", ""]
                });
        }
        if (paletteIndex == Constants.COOL) {
            return
                ColorPalette({
                    name: "cool",
                    background: BACKGROUND,
                    surface: SURFACE,
                    barColors: ["#00FFA3", "#008CFF", "#924DFF", "", "", "", ""]
                });
        }
        ...
        if (paletteIndex == Constants.PASTEL) {
            return
                ColorPalette({
                    ...
                    background: BACKGROUND,
                    surface: SURFACE,
                    barColors: ["#FF9898", "#FFBE99", "#FFF399", "#99FF99", "#99ECFF", "#C799FF", "#FA99FF"]
                });
        }
    ...
    /**
     * @notice Checks if a palette is a genesis palette
     * @param paletteIndex Index of the palette to check
     * @return bool True if the palette is a genesis palette, false otherwise
     */
    function isGenesisPalette(uint8 paletteIndex) internal pure returns (bool) {
        return
            paletteIndex == Constants.REDS ||
            paletteIndex == Constants.YELLOWS ||
            paletteIndex == Constants.GREENS ||
            paletteIndex == Constants.BLUES ||
            paletteIndex == Constants.VIOLETS;
    }
}
